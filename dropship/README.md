...
A single-page, terminal-aesthetic file-drop server. Pure-Python stack
(Flask + waitress + Werkzeug) — no Rust, no C extensions, no build step.
Runs on **Termux/Android** out of the box.
...
- Python **3.10+** (works fine on Termux's Python 3.12)
- pip

All dependencies are pure-Python wheels — no compilation, no Rust toolchain.
